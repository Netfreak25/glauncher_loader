...
void searchHex(char *filepath, char *str3){
    FILE *file,*fileout;
    uint8_t what[] = {0x62, 0x6F, 0x6F, 0x74, 0x2e};
    //uint8_t repl[] = {0x62, 0x68, 0x62, 0x6c, 0x2e};
    //code for bhbl.

    uint8_t repl[] = {0x62, 0x67, 0x72, 0x64, 0x2e};

    size_t count;
    file=fopen(filepath,"rb");
    fileout=fopen(str3,"wb");
...
Result loadsd_payload(char *filepath, u32 *payloadsize)
{
	...
	*payloadsize = filestats.st_size;
	...
	char* str2;
	str2 = "-GRD";
	char * str3 = (char *) malloc( strlen(filepath)+ strlen(str2) );
	...
	if(f==NULL)
	{
		printf("failed to load modified payload.\n");
		searchHex(filepath, str3);
		f = fopen(str3, "r");
		if(f==NULL)
		{
			return errno;
...
Result load_hblauncher()
{
	...
	if(payload_src)
	{
			...
			else
			{
				...
				printf("Loading the downloaded and modified payload from SD now...\n");
				ret = loadsd_payload(payload_sdpath, &payloadsize);
